...
默认兼容画布仍为 384×384。要降低每次分层窗口上传的像素量，可用以下任一种方式试用 256×256 基础画布：

```powershell
.\build\DrawCursor.exe --canvas-256
$env:DRAWCURSOR_CANVAS_SIZE = "256"; .\build\DrawCursor.exe
```

命令行 `--canvas-384` 可覆盖环境变量恢复默认值。基础尺寸只影响兼容模式；当自定义光标尺寸或热点需要更大安全边界时，程序会自动扩展实际画布，避免裁切和越界。由于较小画布会更频繁地重新居中，远程采集出现残影时应恢复 384×384。
